...
export function htmlToMarkdownAST(element: Element, options?: ConversionOptions, indentLevel: number = 0): SemanticMarkdownAST[] {
    ...
    element.childNodes.forEach((childElement) => {
        ...
        if (overriddenElementProcessing) {
            ...
            // Native Agent ID Extraction Hook
            // If the element has a data-agent-id, and it's not one of our custom natively-handled
            // interactive types (input, select, button), we explicitly push the ID marker 
            // right before the element as a pure TextNode.
            ...
            if (agentId && !['input', 'select', 'button'].includes(tagName)) {
                ...
                // Add explicit state visualization for custom ARIA roles
            ...
            if (/^h[1-6]$/i.test(elem.tagName)) {
                ...
                result.push({
                    ...
                    checked: inputElem.type === 'checkbox' || inputElem.type === 'radio' ? inputElem.checked : undefined,
                    disabled: inputElem.disabled,
                    agentId: inputElem.getAttribute('data-agent-id') || undefined
                });
